...
export default async function ListadoPage({
  ...
  return (
    <section>
      ...
      {dbError ? (
        <p className="mt-6 text-sm text-red-600">No se pudo cargar el listado (¿base de datos conectada?).</p>
      ) : (
        <>
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                {rows.length === 0 ? (
                  ...
                  rows.map((r) => (
                    <tr key={r.code} className="border-b border-neutral-100 last:border-0 hover:bg-neutral-50">
                      ...
                      <td className="px-4 py-2 text-neutral-500">{r.provincia ?? "—"}</td>
                    </tr>
                  ))
                )}
              </tbody>
          ...
          {totalPages > 1 && (
            <div className="mt-3 flex items-center justify-between text-sm">
              {page > 0 ? (
                <Link href={href(page - 1)} className="rounded-lg border border-neutral-300 px-3 py-1.5 transition hover:bg-neutral-50">
                  ← Anterior
                </Link>
              ) : (
